...
- 适配 Ombre-Brain 3.6 的 `breath_search` / `breath_advanced` 读取工具，同时保留旧版 `breath` 的滚动部署兼容回退。
- 梦境召回不再受旧 800 token 整桶准入限制，并同时识别 OB 3.6 新目录与旧目录格式。
- OB 梦境回存使用独立的 60 秒写入窗口；确认丢失时通过新版搜索工具按唯一梦境 ID 幂等核验，绝不盲目重复写入。

## 2.9.1 — 2026-08-08

- 正式支持 OpenRouter 作为心潮的自主造梦模型，支持可选的 `HTTP-Referer` 与 `X-OpenRouter-Title` 标识。
- 移除不通用的 `thinking` 请求参数，保留 JSON 结构化输出不受支持时的兼容重试。
- 启用模型时对 Key、模型 slug 和 HTTPS 基址做启动校验，避免错误配置静默产生规则梦。
- Dashboard 将真实模型梦境与规则回退分开标记，并在模型梦境上显示实际模型 slug。
