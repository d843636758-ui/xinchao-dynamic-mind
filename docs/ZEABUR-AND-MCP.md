...
每场自动梦境写入 OB 时都会附带唯一的心潮梦境 ID。如果 `hold` 已经落库但
HTTP/SSE 成功回执丢失，心潮不会重复提交写入，而会按该 ID 只读回查最多三次；
确认存在后记录 `ombre_write_recovered` 并显示“已回存 OB”。

心潮会把每场梦实际采用的 OB 记忆键保存在私有状态中。最近六场梦用过的同一
记忆、与近期梦境余韵共享明显主题片段的目录项，以及心潮自动回存的旧梦，都会
暂时退出候选；其余条目按持久化状态种子轮换。没有新材料时显示“OB 记忆避重复”，
并只依据当前动态状态造梦，不会为了填满上下文反复使用同一条记忆。
